...
# from flashlm.compression.prune.hypernetwork import virtual_mlp_operation, virtual_basic_operation, virtual_att_operation, virtual_block_attn_operation, virtual_block_basic_operation
...
class single_experts_module(nn.Module):
    def __init__(self, mlp_dim, model_dim, experts=8, head_dim=128, attn_flag=False, num_kv_heads=0.5,):
        ...
        # self.experts_embeding = nn.Parameter(torch.Tensor(self.experts, self.emb_dim))
        # nn.init.normal_(self.experts_embeding)
    ...
    def check_head_dim(self, head_dim):
        if self.checked:
            ...
            if self.experts_for_eval[-1,:].sum() < (head_dim/8)*2:
                #print(self.experts_for_eval[-1,:].sum())
                self.actual_moe = True
            else:
                self.actual_moe = False
            return self.actual_moe
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, qk_index=None, unsqueeze_dim=1):
    ...
    if qk_index != None:
        # qk_index = qk_index.expand(cos.shape[0],cos.shape[1],cos.shape[2],qk_index.size(0))
        # cos = torch.gather(cos, 3, qk_index.to(cos.device))
        # sin = torch.gather(sin, 3, qk_index.to(sin.device))
        cos = cos[...,qk_index]
        sin = sin[...,qk_index]
...
class LlamaMLP(nn.Module):
    ...
    def moe_forward(self, x, indices):
        ...
        #print(indices.size())
        # all_weight = torch.cat((self.gate_proj.weight,self.up_proj.weight,self.down_proj.weight.t()),dim=-1)
        # all_weight = all_weight[indices,:]
        # gate_weight, up_weight, down_weight = all_weight[:,:self.hidden_size], all_weight[:,self.hidden_size:2*self.hidden_size], all_weight[:,2*self.hidden_size:].t()
    ...
    def forward(self, x):
        ...
        if self.actual_moe:
            if self.experts_module.experts_list == None:
                self.experts_module.experts_list = [torch.nonzero(self.experts_module.experts_for_eval[i,:]) for i in range(self.experts_module.experts_for_eval.size(0))]
                # self.experts_up = [self.up_proj.weight[indices.squeeze(),:] for indices in self.experts_module.experts_list]
                # self.experts_gate = [self.gate_proj.weight[indices.squeeze(),:] for indices in self.experts_module.experts_list]
                # self.experts_down = [self.down_proj.weight[:,indices.squeeze()] for indices in self.experts_module.experts_list]
            batch_size, sequence_length, hidden_dim = x.shape
            ...
            # # expert_mask = torch.nn.functional.one_hot(binary, num_classes=len(self.experts_module.experts_list)).permute(2, 1, 0)
            # # Loop over all available experts in the model and perform the computation on each expert
            # for expert_idx in range(len(self.experts_module.experts_list)):
            #     expert_indices = self.experts_module.experts_list[expert_idx]
            #     # idx, top_x = torch.where(expert_mask[expert_idx])
            #     top_x = torch.where(binary == torch.Tensor([expert_idx]).to(x.device))[0]
            #     if top_x.shape[0] == 0:
            #         continue
            ...
            #     current_state = x[top_x_list,:].reshape(-1, hidden_dim)
            #     current_hidden_states = self.moe_forward(current_state, expert_indices)

            #     final_hidden_states.index_add_(0, top_x, current_hidden_states.to(x.dtype))
            # down_proj = final_hidden_states.view(batch_size, sequence_length, hidden_dim)
            ...
            if self.config.pretraining_tp > 1:
                ...
                down_proj = self.down_proj((self.act_fn(self.gate_proj(x))*binary) * (self.up_proj(x)*binary))
            return down_proj, router_logits
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        self.experts_module = single_experts_module(self.head_dim, self.hidden_size, head_dim=self.head_dim, attn_flag=True)
    ...
    def forward(
        ...
        **kwargs,
        ...
        query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, qk_index=self.experts_module.qk_index)
...
class LlamaSdpaAttention(LlamaAttention):
    ...
    def forward(
        ...
        #binary_vector =  binary_vector.view(bsz,q_len,-1, 1).transpose(1,2)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if output_router_logits:
            router_logits = torch.cat([router_logits, router_logits_attn], dim=0)
...
def model_replace(model, vectors):
    ...
    for layer_id in range(len(modules)):
        ...
        if type(m).__name__ == 'LlamaMLP':
            mid_vector = vectors[i]
            mid_dim = mid_vector

            #print(vectors[-1])
            experts_module = single_experts_module(mid_dim, m.config.hidden_size, experts=vectors[-1])
            gate_proj = torch.nn.Linear(in_features=m.config.hidden_size, out_features=mid_dim, bias=False)
            up_proj = torch.nn.Linear(in_features=m.config.hidden_size, out_features=mid_dim, bias=False)
            down_proj = torch.nn.Linear(in_features=mid_dim, out_features=m.config.hidden_size, bias=False)
        ...
        if type(m).__name__ == 'LlamaAttention' or type(m).__name__ == 'LlamaFlashAttention2' or type(m).__name__ == 'LlamaSdpaAttention':
            ...
            experts_module = single_experts_module(m.head_dim, m.hidden_size, head_dim=m.head_dim, attn_flag=True, experts=vectors[-1])
            experts_module.register_buffer('qk_index', torch.zeros(pruned_head_dim).to(torch.int64))
            m.experts_module = experts_module
            
            pruned_q_proj = torch.nn.Linear(m.hidden_size, m.num_heads * pruned_head_dim, bias=False)
            pruned_k_proj = torch.nn.Linear(m.hidden_size, m.num_key_value_heads * pruned_head_dim, bias=False)

            m.q_proj = pruned_q_proj
            m.k_proj = pruned_k_proj

            # m.register_buffer('qk_index', torch.zeros(pruned_head_dim).to(torch.int64))
            # print(m.qk_index.size())
            i = i+1
            # experts_module = single_experts_module(m.head_dim, m.hidden_size, head_dim=m.head_dim, attn_flag=True, experts=vectors[-1])
            # m.experts_module = experts_module
...
def router_load_balance_loss(
    ...
    r"""
    Calculate the standard load‑balance (auxiliary) loss for a single router tensor.
    router_logits shape: [num_tokens, num_experts]
    top_k: number of experts allowed per token (e.g. 1 or 2).
    Returns a scalar: mean(top_k_experts * prob * token_frac) * num_experts
    """
    num_experts = router_logits.shape[-1]

    _, selected_experts = torch.topk(router_logits, k=1, dim=-1) # [batch_size X sequence_length, top_k]
    ...
    router_prob_per_expert = torch.mean(router_logits, dim=0) # [num_experts]
    loss = torch.mean(tokens_per_expert * router_prob_per_expert.unsqueeze(0)) # / top_k
    return loss * num_experts

def combined_moe_balance_loss(
    all_router_logits,
    top_k: int = 1,
    layer_weights= None
) -> torch.Tensor:
    r"""
    Compute a combined auxiliary load‑balance loss for multiple MoE router layers.
    - all_router_logits: sequence/tuple of router_logits ([N_tokens, num_experts])
    - top_k: number of experts routed per token (usually 1 or 2)
    - layer_weights: optional per-layer scalar multiplier
    Returns a scalar tensor: ∑_layers (w_l * L_l)
    """
    if layer_weights is None:
        layer_weights = [1.0] * len(all_router_logits)
    assert len(layer_weights) == len(all_router_logits), "layer_weights length mismatch"

    losses = [
        layer_w * router_load_balance_loss(logits, top_k)
        for logits, layer_w in zip(all_router_logits, layer_weights)
    ]
