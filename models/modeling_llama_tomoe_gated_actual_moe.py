...
def gumbel_softmax(logits, temperature, hard_sample=False, return_soft=False, sample=True):
    ...
    y = gumbel_softmax_sample(logits, temperature, sample)
    if not hard_sample:
        return y.view(shape)
...
class SingleMlpRouter(nn.Module):
    def __init__(self, model_dim, experts=8):
        super().__init__()
    ...
    def forward(self, x):
        ...
        hard_max, router_logits = gumbel_softmax(out, temperature=self.T, hard_sample=True, return_soft=True, sample=self.training)
...
class SingleGatedAttnModule(nn.Module):
    def __init__(self, d_model, n_heads, head_dim, rank=128, init_bias=3.0):
        super().__init__()
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        self.use_gated_attn = True
        self.virtual_gated_attn = VirtualGateModule()
    ...
    def _get_output_gate(
        ...
        gate_fn = gated_attn if gated_attn is not None else self.virtual_gated_attn
        if gated_attn is not None:
            gate = gate_fn(hidden_states)
        else:
            gate = gate_fn(hidden_states, self.use_gated_attn)

        if not torch.is_tensor(gate):
            return gate
        if gate.dim() == 4 and gate.shape[1] == q_len:
            return gate.permute(0, 2, 1, 3).contiguous() if head_first else gate
        if gate.dim() == 4 and gate.shape[1] == self.num_heads and not head_first:
            return gate.permute(0, 2, 1, 3).contiguous()
        return gate
    ...
    def forward(
        ...
        **kwargs,
        ...
        gate = self._get_output_gate(hidden_states, q_len, gated_attn)
...
class LlamaFlashAttention2(LlamaAttention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        bsz, q_len, _ = hidden_states.size()
        gate = self._get_output_gate(hidden_states, q_len, gated_attn, head_first=False)
...
LLAMA_START_DOCSTRING = r"""
    ...
    library implements for all its model (such as downloading or saving and resizing the input embeddings).
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            gated_attn = gated_attns[layer_idx] if gated_attns is not None else None
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        cfgs = getattr(config, "tomoe_moe_cfgs", None)
        if cfgs is None:
            cfgs = LlamaForCausalLM.cfgs
        if cfgs is not None:
            model_replace(self, cfgs)
        gate_rank = getattr(config, "tomoe_gated_attn_rank", None)
        if gate_rank is not None:
            attach_gated_attention_modules(
                self,
                gate_rank=gate_rank,
                gate_init_bias=getattr(config, "tomoe_gated_attn_init_bias", 3.0),
            )
